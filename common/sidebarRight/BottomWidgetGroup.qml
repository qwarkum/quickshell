...
Rectangle {
    ...
    property var tabs: [
        ...
        {"type": "todo", "name": "To Do", "icon": "checklist", "widget": todoWidget},
    ...
    Connections {
        ...
        property bool widgetWasCollapsed: root.collapsed

        function onWifiConnectModeChanged() {
            if (Config.wifiConnectMode || widgetWasCollapsed) {
                widgetWasCollapsed = root.collapsed
                setCollapsed(true)
            } else {
                setCollapsed(false)
            }
