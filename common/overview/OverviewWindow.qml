...
Item { // Window
    ...
    property var iconPath: Quickshell.iconPath(AppSearch.guessIcon((windowData?.class)))
    property bool compactMode: 10 * 4 > targetWindowHeight || 10 * 4 > targetWindowWidth
    ...
    ScreencopyView {
        ...
        Rectangle {
            anchors.fill: parent
            radius: Appearance.configs.windowRadius
            color: pressed ? ColorUtils.transparentize(Appearance.colors.secondary, 0.5) : 
                hovered ? ColorUtils.transparentize(Appearance.colors.secondary, 0.7) : 
                ColorUtils.transparentize(Appearance.colors.secondary, 0.9)
